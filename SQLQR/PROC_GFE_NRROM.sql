CREATE PROCEDURE  GETGFEROM  @RNROM AS VARCHAR(6)
AS
BEGIN 
...
SELECT NRROM,
	   ...
	   SERDC,
	   QTVOL,
	   'NUMNFROM' =  (SELECT COUNT(GW1_NRDC) FROM GW1010 WHERE GW1_NRROM = @RNROM + '01' AND D_E_L_E_T_ = ''),
	   CASE 
			WHEN (SELECT GW3_VLDF FROM GW3010 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_VLDF FROM GW3010 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_VLDF FROM GW3020 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_VLDF FROM GW3020 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_VLDF FROM GW3040 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_VLDF FROM GW3040 WHERE GW3_NRDF = [NRDF])
			ELSE NULL
	   END AS VLFATFRETE,
	   TIPO,
	    CASE 
			WHEN (SELECT GW3_CDESP FROM GW3010 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_CDESP FROM GW3010 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_CDESP FROM GW3020 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_CDESP FROM GW3020 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_CDESP FROM GW3040 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_CDESP FROM GW3040 WHERE GW3_NRDF = [NRDF])
			ELSE NULL
	   END AS TPDOCF,
	   CASE 
			WHEN (SELECT GW3_TPDF  FROM GW3010 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_TPDF  FROM GW3010 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_TPDF  FROM GW3020 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_TPDF  FROM GW3020 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_TPDF  FROM GW3040 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_TPDF  FROM GW3040 WHERE GW3_NRDF = [NRDF])
			ELSE NULL
	   END AS TPDF,
	   CASE 
			WHEN (SELECT GW3_TAXAS  FROM GW3010 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_TAXAS  FROM GW3010 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_TAXAS  FROM GW3020 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_TAXAS  FROM GW3020 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_TAXAS  FROM GW3040 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_TAXAS  FROM GW3040 WHERE GW3_NRDF = [NRDF])
			ELSE NULL
	   END AS TAXAS,
	   CASE 
			WHEN (SELECT GW3_VLIMP  FROM GW3010 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_VLIMP  FROM GW3010 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_VLIMP  FROM GW3020 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_VLIMP  FROM GW3020 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_VLIMP  FROM GW3040 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_VLIMP  FROM GW3040 WHERE GW3_NRDF = [NRDF])
			ELSE NULL
	   END AS VLIMP,
	   CASE 
			WHEN (SELECT GW3_PEDAG  FROM GW3010 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_PEDAG  FROM GW3010 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_PEDAG  FROM GW3020 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_PEDAG  FROM GW3020 WHERE GW3_NRDF = [NRDF])
			WHEN (SELECT GW3_PEDAG  FROM GW3040 WHERE GW3_NRDF = [NRDF] AND GW3_SERDF = [SERIEDF]) IS NOT NULL THEN  (SELECT GW3_PEDAG  FROM GW3040 WHERE GW3_NRDF = [NRDF])
			ELSE NULL
	   END AS PEDAG
FROM(
